...
import { IV2PoolProvider, V2SubgraphPool } from '../providers';
...
export async function calculateGasUsed(
  ...
  if (opStackChains.includes(chainId as ChainId)) {
    l2toL1FeeInWei = (
  ...
  // get fee in terms of quote token
    ...
    const nativePools = await Promise.all([
      ...
      getV2NativePool(quoteToken, v2PoolProvider, chilizPools, providerConfig),
    ]);
